...
class tabs {
    ...
    public function btn_save($id = '') {
	  	global $db;
		if ($_SESSION['admin_security'][SECURITY_ID_CONFIGURATION] < 2) throw new \Exception(ERROR_NO_PERMISSION);
    ...
    public function btn_delete($id = 0) {
	  	global $db;
		if ($_SESSION['admin_security'][SECURITY_ID_CONFIGURATION] < 4) throw new \Exception(ERROR_NO_PERMISSION);
