...
// It also needs to check for errors, i.e. missing information, bad data, etc. 
  function updateDatabase($product) {
	global $db;
	// error check input
	if (!$product['sku']) throw new Exception(SOAP_NO_SKU);
	if ($product['action'] <> 'InsertUpdate') throw new Exception(SOAP_BAD_ACTION);
	...
	if ($result->RecordCount() <> 1) throw new Exception(SOAP_BAD_LANGUAGE_CODE . $product['language']);
	...
	if ($result->RecordCount() <> 1) throw new Exception(sprintf(SOAP_BAD_PRODUCT_TYPE, $product_type_name, $product['sku']));
	...
	if ($product['manufacturer'] <> ''){
		...
		if ($result->RecordCount() <> 1) throw new Exception(sprintf(SOAP_BAD_MANUFACTURER, $manufacturer_name, $product['sku']));
	...
	if ($result->RecordCount() <> 1) throw new Exception(sprintf(SOAP_BAD_CATEGORY, $categories_name, $product['sku']));
	...
	if ($result->RecordCount() <> 0) throw new Exception(SOAP_BAD_CATEGORY_A);
	...
	if ($image_data) {
		...
		if (!$handle = fopen(DIR_FS_CATALOG_IMAGES . $full_path, 'wb')) throw new Exception(SOAP_OPEN_FAILED . $full_path);
		if (fwrite($handle, $contents) === false) throw new Exception(SOAP_ERROR_WRITING_IMAGE);
		fclose($handle);
	}
